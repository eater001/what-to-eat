...
const fridgeDateInput = document.getElementById('fridgeDateInput');
const fridgeShelfInput = document.getElementById('fridgeShelfInput');
...
fridgeOcrRunBtn.addEventListener('click', async () => {
    ...
    try {
        ...
        let added = 0;
        const dateVal = fridgeDateInput.value || null;
        const shelfVal = fridgeShelfInput.value || null;
        for (const name of items) {
            ...
            if (!exists) {
                await db.collection('fridge_items').add({
                    ...
                    shelfLife: shelfVal ? Number(shelfVal) : null,
                    createdAt: Date.now()
                });
                added++;
            }
...
async function saveOrUpdateRecipe() {
    ...
    try {
        ...
        if (editingId) {
            ...
            const updateData = { name, steps, image: imageUrl };
...
async function applyManualCategory(recipeId, newCat) {
...
function renderFridge() {
    ...
    fridgeItems.forEach(item => {
        ...
        if (item.purchaseDate && item.shelfLife) {
            const purchase = new Date(item.purchaseDate);
            purchase.setHours(0, 0, 0, 0);
            const expire = new Date(purchase);
            expire.setDate(expire.getDate() + Number(item.shelfLife));
            const diffDays = Math.ceil((expire - today) / (1000 * 60 * 60 * 24));

            const purchaseStr = item.purchaseDate;
            const expireStr = expire.toISOString().slice(0, 10);

            if (diffDays < 0) {
                statusClass = 'expired';
                statusHtml = `<span class="fridge-status expired">⚠️ 已过期 ${Math.abs(diffDays)} 天</span>
                              <span class="fridge-dates">买：${purchaseStr} · 到：${expireStr}</span>`;
            } else if (diffDays <= 3) {
                statusClass = 'soon';
                statusHtml = `<span class="fridge-status soon">⏰ 还剩 ${diffDays} 天</span>
                              <span class="fridge-dates">买：${purchaseStr} · 到：${expireStr}</span>`;
            } else {
                statusHtml = `<span class="fridge-status fresh">✅ 还剩 ${diffDays} 天</span>
                              <span class="fridge-dates">买：${purchaseStr} · 到：${expireStr}</span>`;
            }
        } else if (item.purchaseDate) {
            statusHtml = `<span class="fridge-status">📅 买于 ${item.purchaseDate}</span>`;
        }

        html += `
            <div class="fridge-item ${statusClass}" data-id="${item.id}">
...
fridgeAddBtn.addEventListener('click', async () => {
    ...
    if (added > 0) {
        fridgeInput.value = '';
        fridgeDateInput.value = '';
        fridgeShelfInput.value = '';
        await loadAllData();
        renderFridge();
        showToast(`已添加 ${added} 种食材`, 1500);
    } else {
        showToast('这些食材都已在冰箱里', 1500);
    }
});
